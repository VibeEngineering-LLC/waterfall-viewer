...
# Задача #45: кривую сечения красим в БЕЛЫЙ (не цветом оси) — бирюза/пурпур тонут на рельефе
# viridis и сливаются с рамкой; белый контрастен и на чёрном фоне, и на цветной поверхности.
_PROFILE_RGB = (1.0, 1.0, 1.0)
...
class Waterfall3DView(gl.GLViewWidget):
    ...
    def _apply_plane(self, axis: str, slot: int) -> None:
        ...
        if prof is not None:
            # Задача #41: кривая спектра/ряда на самой секущей плоскости — высота = срез рельефа
            # (prof.z = self._z_surface[...]), рисуется поверх (см. _PROFILE_GL).
            # Задача #45: цвет БЕЛЫЙ (_PROFILE_RGB), а не оси — иначе тонет на рельефе и в рамке.
            line.setData(pos=prof, color=(*_PROFILE_RGB, 1.0), width=3.0)
