...
from awf.ui.knobs import AdjustPanel, _tbin_mult
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _on_adjust_changed(self) -> None:
        ...
        if v["tbin"] != last["tbin"]:                       # Задача #UI-234: масштаб оси t (0 в центре)
            self._view3d.set_x_stretch(_tbin_mult(v["tbin"]))
        self._adj_last = v
    ...
    def _on_loaded(self, sg) -> None:
        ...
        # Задача #DATA-1: отчёт целостности CRC32 (ASWF v4) в статус-строку + диалог при порче.
        integ = self._integrity_suffix(getattr(sg, "integrity_report", None))
    ...
    def _integrity_suffix(rep: dict | None) -> str:
        """Короткий суффикс целостности для статус-строки (Задача #DATA-1)."""
        if not rep:
            return ""
        st = rep.get("status")
        if st == "ok":
            return f"; {tr('CRC32 OK')} ({rep.get('checked', 0)})"
        if st == "corrupt":
            return f"; ⚠ {tr('CRC32: повреждено строк')} {rep.get('bad', 0)}/{rep.get('checked', 0)}"
        if st == "skipped_compressed":
