...
- **Тесты:** `pytest` под headless-Qt — `QT_QPA_PLATFORM=offscreen`, `PYTHONIOENCODING=utf-8`.
  Полный прогон на момент #UI-244 (v0.1.15) при свежем `git clone` — 838 collected:
  835 passed, 3 skipped (`requires_sample` без `sample_data/waterfall_sample.n42`, `.gitignore`).
  +6 passed (844 итого) только с local-only `sample_data/waterfall_sample.n42` +
