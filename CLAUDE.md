...
| Документ | Путь |
|---|---|
| Полные проектные инструкции + журнал задач `#1..#160`, `#REL-*` | [`docs/dev/CLAUDE_NOTES.md`](docs/dev/CLAUDE_NOTES.md) |
...
- **Тесты:** `pytest` под headless-Qt — `QT_QPA_PLATFORM=offscreen`, `PYTHONIOENCODING=utf-8`.
  Полный прогон на момент `#160` — 603 passed (локально, со scipy + sample-файлом).
