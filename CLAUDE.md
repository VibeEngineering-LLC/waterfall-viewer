...
- **Тесты:** `pytest` под headless-Qt — `QT_QPA_PLATFORM=offscreen`, `PYTHONIOENCODING=utf-8`.
  Полный прогон на момент #REL-16 (v0.1.16) при свежем `git clone` — 847 collected:
  844 passed, 3 skipped (`requires_sample` без `sample_data/waterfall_sample.n42`, `.gitignore`).
  +6 passed (850 итого) только с local-only `sample_data/waterfall_sample.n42` +
