...
- **Тесты:** `pytest` под headless-Qt — `QT_QPA_PLATFORM=offscreen`, `PYTHONIOENCODING=utf-8`.
  Полный прогон на момент #REL-17 (v0.1.17) при свежем `git clone` — 849 collected:
  846 passed, 3 skipped (`requires_sample` без `sample_data/waterfall_sample.n42`, `.gitignore`).
  +6 passed (852 итого) только с local-only `sample_data/waterfall_sample.n42` +
