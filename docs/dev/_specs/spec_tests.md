...
- Сгенерируй ОДИН файл Python: содержимое `tests/test_data_layer.py` (pytest).
- Выводи ТОЛЬКО сырой код Python. БЕЗ markdown-ограждений (```), БЕЗ пояснений, БЕЗ текста до/после.
- Первая строка — docstring модуля. Комментарии — на русском, идентификаторы — на английском.
- Зависимости: pytest, numpy, стандартная библиотека. Запуск: `python -m pytest` из корня проекта.
- НЕ оставляй TODO/pass. Каждый тест полностью реализован и осмысленно проверяет инвариант.
...
КОНСТАНТЫ (опорные значения — независимый оракул, выведенный из байтов реального файла):
...
ВСПОМОГАТЕЛЬНЫЙ ГЕНЕРАТОР well-formed потока CountedZeroes (для fuzz):
...
3) def test_decode_vec_matches_scalar_fuzz():
...
4) def test_calibration_polynomial():
...
5) def test_channel_of_energy_roundtrip():
   cal = Calibration.from_coeff_string(" ".join(repr(c) for c in COEFFS))
   en = cal.energies(8192)
