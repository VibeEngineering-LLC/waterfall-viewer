...
- Сгенерируй ОДИН файл Python: содержимое модуля `awf/ui/panels.py`.
...
- Зависимости: numpy, pyqtgraph, PySide6 (QtCore, QtWidgets). НЕ импортируй scipy/pandas/PyOpenGL.
...
НАЗНАЧЕНИЕ:
Два виджета: (1) HeatmapPanel — 2D-карта Время×Энергия (ImageItem) с прямоугольной выборкой (ROI);
(2) SlicePanel — графики срезов/сечений: спектр (Энергия→Отсчёты) и временной ряд (Время→Отсчёты).
HeatmapPanel при изменении ROI испускает сигнал; MainWindow связывает его со SlicePanel.

КОНТРАКТ С МОДЕЛЬЮ Spectrogram (вызывать ТОЛЬКО эти атрибуты; других НЕ выдумывать):
  sg.n_slices (int), sg.n_channels (int)            — свойства
  sg.counts                                          — 2D ndarray (n_slices, n_channels)
  sg.energies() -> 1D ndarray (n_channels)           — энергии каналов, кэВ
  sg.time_offsets_s -> 1D ndarray (n_slices)         — секунды от начала
  sg.energy_spectrum(i) -> 1D ndarray (n_channels)   — спектр среза времени i
  sg.band_time_series(ch_lo, ch_hi) -> 1D (n_slices) — сумма по полосе каналов на каждый срез
  sg.sum_spectrum(t_lo, t_hi) -> 1D (n_channels)     — спектр, просуммированный по окну времени
  sg.total_spectrum() -> 1D (n_channels)             — полный интегральный спектр
  sg.roi_sum(t_lo, t_hi, ch_lo, ch_hi) -> int        — сумма отсчётов в прямоугольной выборке
  sg.downsample(max_time, max_chan, method="max") -> (counts_ds 2D, t_centers 1D, ch_centers 1D)

================ ВИДЖЕТ 1: HeatmapPanel ================
...
ВАЖНО ПО API pyqtgraph/PySide6 (соблюдать дословно):
- pg.ImageItem.setImage(img, axisOrder="row-major") => ось0 массива = строки = вертикаль (Y),
  ось1 = столбцы = горизонталь (X).
- pg.RectROI([x,y],[w,h]); .pos() и .size() возвращают объекты с .x()/.y(); сигнал
  sigRegionChangeFinished без аргументов.
- QtCore.Signal(int,int,int,int) — объявляется на уровне класса QWidget.
- НЕ создавать QApplication внутри модуля; НЕ вызывать .show().
