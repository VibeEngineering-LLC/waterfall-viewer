...
- Сгенерируй ОДИН файл Python: содержимое модуля `awf/io/n42_loader.py`.
...
- Зависимости: стандартная библиотека + numpy + lxml (`from lxml import etree`). НЕ импортируй pandas/scipy.
...
НАЗНАЧЕНИЕ:
Потоковый (streaming) загрузчик waterfall-спектрограммы в формате ANSI N42.42-2011 (RadInstrumentData).
Файл может быть огромным (запись сутки и более → десятки тысяч спектров, сотни МБ): парсить через
lxml.etree.iterparse с освобождением памяти, НЕ загружать XML целиком в дерево.
...
ДЕКОДЕР CountedZeroes — ДВЕ независимые реализации (НЕ объединять; это намеренно для перекрёстной проверки):

Формат CountedZeroes (сжатие нулевых пробегов в N42):
  поток токенов; НОЛЬ — маркер: следующий токен = количество нулей подряд;
  ненулевой токен — литерал (само значение, одно вхождение).
  Корректный поток: каждый ноль — маркер, за ним count>=1. Пример: "5 0 3 7" -> [5,0,0,0,7].
